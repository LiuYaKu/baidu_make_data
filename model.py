...
class TrieNode(object):
    ...
    def find_word(self, N ,stop_word,jieba_dict,l_zre):
        # 通过搜索得到互信息
        # 例如: dict{ "a_b": (PMI, 出现概率), .. }
        bi = self.search_bi()
        # 通过搜索得到左右熵
        ...
        for key,values in bi.items():
            ...
            t_key = ""
            temp_x=[]
            if tt[0] in jieba_dict:
                t_key += jieba_dict[tt[0]]
                temp_x.append(jieba_dict[tt[0]])
            else:
                t_key += "x"
                temp_x.append("x")
            t_key += "_"
            if tt[1] in jieba_dict:
                t_key += jieba_dict[tt[1]]
                temp_x.append(jieba_dict[tt[1]])
            else:
                t_key += "x"
                temp_x.append("x")
            if t_key in l_zre:
            ...
            if temp_x[0]=="uj":
                continue
            if temp_x[1]=="uj" and "a" not in temp_x[0]:
                continue
            if tt[0]== "的":
                continue
            if tt[0] == tt[1]:
                continue
            if tt[0] in stop_word or tt[1] in stop_word:
                continue
            d = "".join(tt)
